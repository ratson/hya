...
async function generateResources(config) {
  ...
  const args = _.flatten(
    await Bluebird.map(['icon', 'splash'], async resKey => {
      const resPath = path.resolve(
        config.cwd,
        config[resKey] || `hya/${resKey}.png`,
      )
      if (await fse.pathExists(resPath)) {
        resType = resKey
        return [`--${resKey}-source`, resPath]
      }
      return []
    }),
  )
  if (args.length === 2) {
    args.push('--type', resType)
  }
  await execa(require.resolve('cordova-res/bin/cordova-res'), args, {
    cwd: config.cordovaDir(),
    stdio: 'inherit',
  })
}
